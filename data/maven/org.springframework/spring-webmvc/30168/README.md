## Overview
[`org.springframework:spring-webmvc`](http://search.maven.org/#search%7Cga%7C1%7Ca%3A%22spring-webmvc%22)
Affected versions of this package are vulnerable to Directory traversal attack which allows remote attackers to read arbitrary files via a crafted URL.

## References
- [Pivotal Security](http://www.pivotal.io/security/cve-2014-3578)
- [Jira Issue](https://jira.spring.io/browse/SPR-12354)
- [JVNDB](http://jvndb.jvn.jp/en/contents/2014/JVNDB-2014-000054.html)
- [Redhat Bugzilla](https://bugzilla.redhat.com/show_bug.cgi?id=1131882)
